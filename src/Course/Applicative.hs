...
replicateA ::
  ...
  -> f a
  -> f (List a)
replicateA n fa = sequence $ take n $ repeat fa
...
filtering f (x:.xs) = lift2 (\b xs' -> if b then x:.xs' else xs') (f x) rest
  where rest = filtering f xs
