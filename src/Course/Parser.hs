...
character ::
  Parser Char
character = P p
  where p Nil = ErrorResult Failed
        p (c:.cs) = Result cs c
...
list1 ::
  Parser a
  -> Parser (List a)
list1 p = flbindParser p (\r -> flbindParser (list p) (\rs -> valueParser (r:.rs)))
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy g = flbindParser character result
  where result c
          | g c = valueParser c
...
natural ::
  Parser Int
natural = flbindParser (list1 digit) convert
  where convert s = case read s of
          Empty -> failed
...
sequenceParser (p:.ps) = 
  flbindParser p (\r -> 
    flbindParser (sequenceParser ps) (\rs ->
      valueParser (r:.rs)))
...
firstNameParser = 
  flbindParser upper (\r -> 
    flbindParser (list (lower ||| upper)) (\rs ->
      valueParser (r:.rs)))
...
surnameParser = 
  flbindParser upper (\r -> 
    flbindParser (thisMany 5 lower) (\rs ->
      flbindParser (list lower) (\rss ->
        valueParser ((r:.rs) ++ rss))))
...
phoneParser = 
  flbindParser digit (\d -> 
    flbindParser phoneBodyParser (\xs ->
      flbindParser (is '#') (\_ ->
        valueParser (d:.xs))))
...
personParser = do
  ...
  spaces1 
  ph <- phoneParser
  pure $ Person a fname sname smok ph
